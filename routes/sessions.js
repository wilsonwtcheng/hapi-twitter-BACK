 ...
 server.route([
 	...
 	{ 
 		...
 		handler: function(request, reply) { //ALWAYS request reply here.
			...
			//read the payload
			var user = request.payload.userInfo; //userInfo is for the stuff that goes in postman, eg. userInfo[username] : wilzon	
			...
			db.collection("users").findOne({"username": user.username}, function(err, userMongo) {
				...
				Bcrypt.compare(user.password, userMongo.password, function(err, matched){
					...
					db.collection("sessions").insert(newSession, function(err, writeResult) {
						...
						request.session.set("hapi_twitter_session", { //name of hapi twitter session doesn't really matter. We are setting it to this name. WE WILL BE REFERENCING THIS LATER!
